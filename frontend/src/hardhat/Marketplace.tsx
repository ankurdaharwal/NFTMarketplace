...
>([defaultSigner, () => {}]);
const defaultSymfoniContext: ContextInterface = {
...
export const NFTMarketplaceContext = React.createContext<ContextNFTMarketplace>(
  emptyContract
);
export const NFTMarketplaceStorageContext = React.createContext<ContextNFTMarketplaceStorage>(
  emptyContract
);
export const PaymentTokenContext = React.createContext<ContextPaymentToken>(
  emptyContract
);
...
export const Marketplace: React.FC<ContextProps> = ({
  ...
  const [NFTMarketplace, setNFTMarketplace] = useState<ContextNFTMarketplace>(
    emptyContract
  );
  const [
    NFTMarketplaceStorage,
    setNFTMarketplaceStorage,
  ] = useState<ContextNFTMarketplaceStorage>(emptyContract);
  const [PaymentToken, setPaymentToken] = useState<ContextPaymentToken>(
    emptyContract
  );
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const doAsync = async () => {
      ...
      setLoading(true);
      setMessages((old) => [...old, "Initiating React"]);
      ...
      setMessages((old) => [
        ...old,
        "Using " + providerObject.hardhatProviderName,
      ]);
      ...
      setSigner(_signer);
      setMessages((old) => [...old, "Using signer"]);
      ...
      return finishWithContracts("Completed context initialization.");
    };
    doAsync();
    return () => {
      subscribed = false;
  ...
  const getNFTMarketplace = (
    _provider: providers.Provider,
    _signer?: Signer
  ) => {
    const contractAddress = NFTMarketplaceDeployment.receipt.contractAddress;
    const instance = _signer
      ? NFTMarketplace__factory.connect(contractAddress, _signer)
      : NFTMarketplace__factory.connect(contractAddress, _provider);
    const contract: ContextNFTMarketplace = {
      instance: instance,
      factory: _signer ? new NFTMarketplace__factory(_signer) : undefined,
    };
    return contract;
  };
  const getNFTMarketplaceStorage = (
    _provider: providers.Provider,
    _signer?: Signer
  ) => {
    let instance = _signer
      ? NFTMarketplaceStorage__factory.connect(
    ...
    const contract: ContextNFTMarketplaceStorage = {
      instance: instance,
      factory: _signer
        ? new NFTMarketplaceStorage__factory(_signer)
        : undefined,
    };
    return contract;
  };
  const getPaymentToken = (_provider: providers.Provider, _signer?: Signer) => {
    ...
    const instance = _signer
      ? PaymentToken__factory.connect(contractAddress, _signer)
      : PaymentToken__factory.connect(contractAddress, _provider);
    const contract: ContextPaymentToken = {
      instance: instance,
